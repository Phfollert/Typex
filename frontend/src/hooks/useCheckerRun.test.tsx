...
beforeEach(() => {
  ...
  vi.stubGlobal(
    'fetch',
    vi.fn((url: string) => {
      if (url === '/api/checkers') return Promise.resolve({ ok: true, json: async () => CHECKERS });
  ...
  it('a stale run finishing first neither commits results nor clears "checking" under the newer run', async () => {
    const { result, rerender } = renderHook(
      ({ files }: { files: Record<string, string> }) =>
        useCheckerRun({ files, targetVersion: 'py312', isReady: true, canRun: true }),
      { initialProps: { files: { 'a.py': 'A' } } }
    );

    // First advance loads the checkers (which schedules the debounce); the
    // second lets the debounce elapse so run A starts.
    await act(async () => vi.runAllTimersAsync());
    await act(async () => vi.runAllTimersAsync());
    expect(result.current.field.selectedCheckerIds).toEqual(['c1']);
    expect(pendingRuns).toHaveLength(1);
    expect(result.current.config.isSubmitting).toBe(true);
